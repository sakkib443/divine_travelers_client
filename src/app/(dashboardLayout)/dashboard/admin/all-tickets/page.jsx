            ...
            <div className="p-4 lg:p-6 max-w-6xl mx-auto space-y-5">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div>
                        <h1 className="text-2xl font-black text-gray-800 flex items-center gap-2">
                            <LuPlane className="text-[#0F3C53]" /> All Tickets
                        </h1>
                    ...
                    <div className="flex gap-2">
                        ...
                        <Link href="/dashboard/admin/ticket-generator"
                            className="flex items-center gap-2 px-5 py-2 bg-[#0F3C53] text-white rounded-xl text-sm font-bold hover:bg-[#0F3C53] shadow-lg shadow-blue-200">
                ...
                <div className="relative">
                    ...
                    <input type="text" value={search} onChange={e => setSearch(e.target.value)}
                        ...
                        className="w-full pl-10 pr-4 py-2.5 bg-white border border-gray-200 rounded-xl text-sm outline-none focus:border-[#0F3C53] transition-all" />
                </div>

                {/* Loading */}
                {loading ? (
                    <div className="flex items-center justify-center py-20">
                        <FiLoader size={32} className="animate-spin text-[#0F3C53]" />
                    </div>
                ) : filtered.length === 0 ? (
                    <div className="text-center py-20">
                        ...
                        <Link href="/dashboard/admin/ticket-generator"
                            className="inline-flex items-center gap-2 mt-4 px-5 py-2 bg-[#0F3C53] text-white rounded-xl text-sm font-bold hover:bg-[#0F3C53]">
                                ...
                                <motion.div key={t._id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                                    ...
                                    <div className="flex flex-col sm:flex-row">
                                        ...
                                        <div className="bg-gradient-to-br from-blue-600 to-blue-800 text-white p-4 sm:w-48 flex flex-col items-center justify-center gap-1">
                                            ...
                                            <div className="text-[10px] text-[#0F3C53]/20 font-semibold">{flight?.airline} {flight?.flightNo}</div>
                                            <div className="text-[10px] text-[#0F3C53] mt-1">{flight?.departDate}</div>
                                        </div>
                                            ...
                                            <div className="flex items-start justify-between">
                                                <div>
                                                    <div className="flex items-center gap-2 mb-1">
                                                        <LuUser size={12} className="text-[#0F3C53]" />
                                                        <span className="text-sm font-bold text-gray-800">{passenger?.name || "No Name"}</span>
                                                        <span className="px-2 py-0.5 bg-[#0F3C53]/5 text-[#0F3C53] rounded text-[10px] font-bold">{passenger?.type || "ADT"}</span>
                                                    </div>
                                        ...
                                        <div className="flex sm:flex-col items-center gap-2 p-3 sm:justify-center border-t sm:border-t-0 sm:border-l border-gray-100">
                                            <Link href={`/dashboard/admin/ticket-generator?id=${t._id}`}
                                                className="flex items-center gap-1.5 px-3 py-1.5 bg-[#0F3C53]/5 text-[#0F3C53] rounded-lg text-[11px] font-bold hover:bg-[#0F3C53]/10 transition-all">
